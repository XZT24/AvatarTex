...
def save_obj_mesh_with_uv(mesh_path, verts, faces, uvs, uvfaces, usemtl=False, mtl_tmp=None):
    file = open(mesh_path, 'w')

    if usemtl:
        if mtl_tmp is not None:
            shutil.copyfile(mtl_tmp, mesh_path[:-4] + '.mtl')
        else:
            mtlfile=open(mesh_path[:-4] + '.mtl', 'w')

            name = mesh_path.split('/')[-1][:-4] 
            mtlfile.write('map_Kd ' + name + '.png\n')
            mtlfile.write('newmtl material_0\n')
            mtlfile.close()
            
            file.write('mtllib ' + name + '.mtl\n')
            
    file.write('mtllib ' + mesh_path.split('/')[-1][:-4] + '.mtl\n')    
    for v in verts:
        file.write('v %.4f %.4f %.4f\n' % (v[0], v[1], v[2]))
    for vt in uvs:
        file.write('vt %.4f %.4f\n' % (vt[0], vt[1]))
    ...
    for f in faces:
        f_plus = f + 1
        uvf_plus = uvfaces[count,:] + 1
        file.write('f %d/%d %d/%d %d/%d\n' % (f_plus[0], uvf_plus[0], f_plus[1], uvf_plus[1], f_plus[2], uvf_plus[2]))
        count += 1
    file.close()
...
def save_obj_mesh_with_uv_normals(mesh_path, verts, faces, uvs, uvfaces, normals=None, usemtl=False, mtl_tmp=None):
    # File name without extension
    name = os.path.basename(mesh_path)[:-4]
    
    if usemtl:
        if mtl_tmp is not None:
            shutil.copyfile(mtl_tmp, os.path.join(os.path.dirname(mesh_path), f"{name}.mtl"))
        else:
            with open(os.path.join(os.path.dirname(mesh_path), f"{name}.mtl"), 'w') as mtlfile:
                mtlfile.write(f"map_Kd {name}.jpg\n")
                mtlfile.write("newmtl material_0\n")
    ...
    with open(mesh_path, 'w') as file:
        ...
        # Write UV coordinates to the file
        for vt in uvs:
            file.write(f"vt {vt[0]:.4f} {vt[1]:.4f}\n")

        # Write vertex normals if provided
        if normals is not None and len(normals) > 0:
            for vn in normals:
                file.write(f"vn {vn[0]:.4f} {vn[1]:.4f} {vn[2]:.4f}\n")
        
        file.write("usemtl material_0\n")
        
        # Write faces to the file
        for idx, f in enumerate(faces):
            f_plus = [x + 1 for x in f]
            uvf_plus = [x + 1 for x in uvfaces[idx]]
            if normals is not None and len(normals) > 0:
                file.write(f"f {f_plus[0]}/{uvf_plus[0]}/{f_plus[0]} {f_plus[1]}/{uvf_plus[1]}/{f_plus[1]} {f_plus[2]}/{uvf_plus[2]}/{f_plus[2]}\n")
            else:
                file.write(f"f {f_plus[0]}/{uvf_plus[0]} {f_plus[1]}/{uvf_plus[1]} {f_plus[2]}/{uvf_plus[2]}\n")
...
def save_obj_mesh_with_rgb(mesh_path, verts, faces, rgbs):
    ...
    for v in verts:
        ...
        file.write(' %.4f %.4f %.4f\n' % (rgbs[num][0], rgbs[num][1], rgbs[num][2]))
        num += 1
    for f in faces:
        f_plus = f + 1
        file.write('f %d %d %d\n' % (f_plus[0], f_plus[1], f_plus[2]))
...
def load_obj_mesh(mesh_file, with_normal=False, with_texture=False):
    ...
    for line in f:
        ...
        if values[0] == 'v':
            ...
            vertex_data.append(v)
            if len(values)>5:
                color_data.append(list(map(float, values[4:])))
        elif values[0] == 'vn':
            vn = list(map(float, values[1:4]))
            norm_data.append(vn)
        elif values[0] == 'vt':
